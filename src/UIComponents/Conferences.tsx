...
const Conferences: React.FC = () => {
  ...
  return (
    <div className={`w-full flex flex-col items-center justify-center mb-10 roboto-regular ${isXXS || isXS || isSM?"mt-16":isMD || isLG || isXL?"mt-22":"mt-30"} `}>
      <NavigationComponent />
        ...
        <div className="min-w-[1000px] grid grid-cols-[auto_1fr_1fr_1fr_1fr_1fr_1fr] rounded-[10px] overflow-hidden border border-white">
          ...
          <div className="contents">
            {Object.keys(tableData[0]).map((key) => (
              <div
                key={key}
                className={`border border-white px-4 py-3 flex items-center justify-center text-[16px] md:text-[20px] font-normal tracking-[0%] leading-snug roboto-regular text-center ${key === "Conference Name" ? "max-w-[300px] whitespace-normal break-words" : ""}`}
              >
                {key === "S no" ? "S no." : key}
              </div>
            ))}
          ...
          {tableData.map((row) => (
            <div key={row["S no"]} className="contents">
              {Object.values(row).map((value, index) => (
                <div
                  key={`${row["S no"]}-${index}`}
                  className={`border border-white px-4 py-3 text-center align-middle text-[14px] md:text-[18px] font-normal tracking-[0%] leading-normal roboto-regular ${index === 2 ? "max-w-[200px] whitespace-normal break-words" : ""}`}
                >
                  {index === 0 ? `${value}.` : value}
                </div>
              ))}
      ...
      {/* Banner Section */}
      <div className={`w-full flex ${isXXS || isXS || isSM?"mt-10":isMD || isLG || isXL?"mt-17": "mt-27"} flex-col items-center`}>
